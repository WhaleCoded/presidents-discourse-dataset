...
class PresidentsDataset:
    def __init__(self, path_to_data = DEFAULT_PATH, primary_categories = None, sub_categories = None, max_length = None):
    ...
    def _load_data(self):
        ...
        file_paths = [os.path.join(self.path_to_data, file_name) for file_name in os.listdir(self.path_to_data) if os.path.isfile(os.path.join(self.path_to_data, file_name)) and file_name.endswith(".json")]
        for file_path in tqdm(file_paths, desc="Loading data"):
            with open(file_path, "r") as f:
                ...
                if self.primary_categories is None and self.sub_categories is None:
                    ...
                    if all([category in json_data["categories"]["primary"] for category in self.primary_categories]):
                        data.append(json_data)
                elif all([category in json_data["categories"]["sub"] for category in self.sub_categories]):
